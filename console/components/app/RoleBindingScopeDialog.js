/**
 * Created by xifeng on 2017/01/03.
 * edit or create app Role bindings(scope) dialog
 */
/*jshint esversion: 6 */
...
class RoleBindingScopeDialog extends Component {
    ...
    render() {
        const title = this.props.data ? `${util.t("app.perm.binding.scope.title")} : ${this.props.target}`
                    : util.t("app.perm.binding.scope.title");
        return (
            <ModalDialog show={this.props.show} title={title} height="auto" width="800px" style={{textAlign:"left"}}>
                {
                    this.props.clientId && this.props.role && this.props.data &&
                    <div>
                        {this.props.targetType === "USER" &&
                        <div className="align-left mb-10">
                            {util.t("app.perm.binding.tip.scope")}
                        </div>
                        }
                        <BindingScopeOrgTree
