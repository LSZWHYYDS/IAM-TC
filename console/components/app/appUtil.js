...
let appUtil = {
     ...
     */
    trustable(app) {
        return app && app.application_type && ["native"].includes(app.application_type.toLowerCase()) ;
    },
    ...
    /**
     * check if an application can have a client_secret
     * @param {object} appDetail application detail
     */
    hasClientSecret(appDetail) {
        return appDetail && (
            appDetail.client_secret ||
            (appDetail.application_type && ["web", "trusted", "cli"].includes(appDetail.application_type.toLowerCase())));
    },
    ...
    canBeAuthorizationDelegated(appType) {
        return appType && "native" === appType.toLowerCase();
    },
